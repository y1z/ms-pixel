...
func _simple_method(start_: Vector2i, end_: Vector2i) -> void:
	...
	for x in range(minimum_x, maximum_x):
		...
		raw_image.set_pixel(floori(corrected.x), floori(corrected.y), UserData.current_color)
...
func put_pixel() -> void:
	...
	match UserData.draw_modes:
		...
		_:
			printerr("UN-HANDLED CASE %s " % UserData.DrawModes.keys()[UserData.draw_modes])
